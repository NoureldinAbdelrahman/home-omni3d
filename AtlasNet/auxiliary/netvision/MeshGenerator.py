    ...
    def read_obj(self):
        with open(self.obj_path, "r") as in_file:
            ...
            for line in in_file:
                ...
                if len(line_s)==0:
                    continue
                if line_s[0] == "ply" or line_s[0] == "format" or line_s[0] == "element" or line_s[0] == "property":
                    in_ply = True
                    continue
                if line_s[0] == "end_header":
                    in_ply = True
                    continue
                if in_ply and line_s[0] not in ("v", "f") and line_s[0] not in ("ply", "format"):
                    # ASCII PLY body: "x y z" or "n i0 i1 i2"
                    try:
                        if len(line_s) == 3 and all(self._is_float(x) for x in line_s):
                            points.append([float(x) for x in line_s])
                        elif len(line_s) >= 4:
                            faces.append([int(x) - 1 for x in line_s[1:4]])
                    except ValueError:
                        continue
                    continue
                if line_s[0] == "v":
